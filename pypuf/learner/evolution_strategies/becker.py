...
class Reliability_based_CMA_ES():
    ...
    def learn(self):
        epsilon = self.learn_epsilon(self.instance)
        terminate = False
        while not terminate:
            ...
            measured_rels = self.measure_rels(self.instance, challenges,
                                              self.challenge_num, self.repeat, epsilon)
            sorted_individuals = self.fitness(challenges, self.challenge_num,
                                              measured_rels, self.individuals, epsilon)
            parent = self.get_parent(sorted_individuals, self.parent_size,
                                     self.priorities)
            self.mean = self.update_mean(self.mean, self.step_size, parent)
            self.path_cm = self.cumulation_for_cm(self.path_cm, self.c_c,
                                                  self.path_ss, self.instance.n, self.mu_w, parent)
            self.path_ss = self.cumulation_for_ss(self.path_ss, self.c_sigma,
                                                  self.mu_w, self.cov_matrix, parent)
            cm_mu = self.get_cm_mu(sorted_individuals, self.parent_size,
                                   self.priorities)
            self.cov_matrix = self.update_cm(self.cov_matrix, self.c_1,
                                             self.c_mu, self.path_cm, cm_mu)
            self.step_size = self.update_ss(self.step_size, self.c_sigma,
                                            self.d_sigma, self.path_ss)
    ...
    def measure_rels(instance, challenges, challenge_num, repeat, epsilon):
        ...
        for i, challenge in enumerate(challenges):
            ...
            if cumulation_ratio > epsilon and cumulation_ratio <= 0.5 or \
                                    (1 - cumulation_ratio) > epsilon and (1 - cumulation_ratio) <= 0.5:
                measured_rels[i] = 1
