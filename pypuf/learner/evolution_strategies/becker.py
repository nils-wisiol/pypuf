...
class Reliability_based_CMA_ES():
    ...
    def learn(self):
        ...
        fitness_function = self.get_fitness_function(self.challenges, self.measured_rels)
    ...
    @staticmethod
    def get_fitness_function(challenges, measured_rels):
    ...
    @staticmethod
    def is_different_LTF(new_LTF, different_LTFs, num_of_LTFs, challenges):
        # returns True iff new_LTF is different from previously learned LTFs
        if num_of_LTFs == 0:
            return True
        weight_arrays = different_LTFs[:num_of_LTFs, :]
        new_LTFArray = LTFArray(new_LTF[np.newaxis, :-1], LTFArray.transform_atf, LTFArray.combiner_xor)
        different_LTFArrays = __class__.build_LTFArrays(weight_arrays[:, :-1])
    ...
    @staticmethod
    def set_pole_of_LTFs(different_LTFs, challenges, common_responses):
