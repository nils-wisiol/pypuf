...
class Reliability_based_CMA_ES():
    ...
    def __init__(self, instance, pop_size, parent_size, priorities, challenge_num, repeat, unreliability):
    ...
    def learn(self):
        ...
        self.epsilon = self.learn_epsilon(self.instance, self.unreliability)
    ...
    def is_different_LTF(self):
        ...
        new_LTFArray = LTFArray(self.new_LTF, self.transform, self.combiner)
        ...
        for i, challenge in enumerate(challenges):
            responses[0, i] = new_LTFArray.eval(challenge)
            for j, current_LTF in enumerate(different_LTFs):
                responses[j+1, i] = current_LTF.eval(challenge)
    ...
    def set_pole_of_LTFs(instance, different_LTFs):
        ...
        challenge_num = 100
        responses = np.empty(2 ** instance.k + 1, challenge_num)
        for i, permutation in enumerate(itertools.product((-1, 1), repeat=instance.k)):
            weight_arrays = (different_LTFs.T * permutation).T
            xor_LTFArray = LTFArray(weight_arrays, LTFArray.combiner_xor,
                                    LTFArray.transform_id)
            challenges = tools.sample_inputs(instance.n, challenge_num)
            for j, challenge in enumerate(challenges):
                responses[0, j] = instance.eval(challenge)
                responses[i + 1, j] = xor_LTFArray.eval(challenge)
        differences = np.empty(np.shape(responses)[0] - 1)
        for l in range(1, np.shape(responses)[0]):
            differences[l - 1] = np.sum(np.abs(responses[0, :] - responses[l, :]))
        sorted_indices = np.argsort(differences)
        sorted_LTFs = different_LTFs[sorted_indices[::]]
        return sorted_LTFs[0]

    @staticmethod
    def is_correlated(responses):
        (num_of_LTFs, num) = np.shape(responses)
        for i in range(1, num_of_LTFs):
            differences = np.sum(np.abs(responses[0, :] - responses[i, :]))
            if differences < 0.3*num or differences > 0.7*num:
                return True
        return False
